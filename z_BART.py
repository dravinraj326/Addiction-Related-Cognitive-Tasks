...
def bart(save_filename, num_runs):
    ...
    win = visual.Window(size = [1200, 800], fullscr=False, units='pix', color=[200, 200, 200])
    scr_len = win.size[0]
    scr_hgt = win.size[1]
    x_mid = scr_len / 32
    y_mid = scr_hgt / 32
    ...
    num_control_trials=5
    #Control Experiment
    while num_control_trials > 0:
        write_trigger('start_control', a)
        if num_control_trials == 5:
        ...
        for i in range(1, max_num_inflations + 1):
            ...
            visual.TextStim(win, text=rwd_str, pos=(-0.5 * scr_len / 2, 0.85 * scr_hgt / 2), color=text_clr,
